...
class TBDatabase:
    ...
    def get_tv_show_with_seasons_by_state(self, state: str) -> list:
        ...
        self.connection.row_factory = dict_factory
        cur = self.connection.cursor()
        cur.execute("SELECT * FROM tv_shows_with_seasons_view WHERE state=?", (state,))
        return cur.fetchall()
    ...
    def add_tv_show_season(self, show_id: str, season_number: str, season_number_episodes: int):
        ...
        self.connection.commit()
        return  cur.execute('SELECT last_insert_rowid()').fetchone()
    ...
    def get_season_id(self, show_id: str, season_number: int):
        ...
        row = cur.execute(
            """
            SELECT id FROM tv_show_seasons WHERE show_id=?, season_id=?
            """,
