...
class TVShowProbe:
            ...
            # Update episodes added during the season anuncements
                ...
                for episode in season:
                    ...
                    self.db.update_show_season(
                        season_id, season_number_episodes=len(season.keys()))
    ...
    def is_season_complete(self, episodes: list):
        ...
        last_episode_date = datetime.strptime(
            episodes[last_episode_number-1]['air_date'], "%d %b. %Y")
    ...
    def update(self) -> None:
        ...
        for episode in episodes:
            ...
            # If there's episodes with no hash, it means that the season was downloaded as a whole
            if not episode_hash:
                break
            ...
            torrents = self.qbit.torrents_info(status_filter=None, hashes=episode_hash)
            for torrent in torrents:
                # State changed from paused,  therefore reusme
                if episode_state != self.db.states.PAUSED and 'paused' in torrent["state"].lower():
                    self.qbit.resume(episode_hash)

                # Remove the torrent if it is older than the retention period
                if episode_state == self.db.states.SEEDING:
                    time_since_added_sec = int(time.time()) - int(torrent["added_on"])
                    if time_since_added_sec > self.retention_preiod_sec:
                        self.qbit.delete(episode_hash)
                        self.db.update_tv_show_season_episode(episode_id, state=self.db.states.COMPLETED)
                # Change the torrent state if it finished the download and it is now uploading
                elif episode_state == self.db.states.DOWNLOADING and torrent["state"] == "uploading":
                    self.db.update_show_season(episode_id, state=self.db.states.SEEDING)
                # Stop download for torrents stopped
                elif episode_state == self.db.states.PAUSED and not 'paused' in torrent["state"].lower():
                    self.qbit.stop(episode_hash)
