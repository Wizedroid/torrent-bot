...
class Probe(ABC):
    ...
    def start(self) -> None:
        ...
        try:
            ...
            logging.error(f"Failed to find show information on imdb (imdbid={show['imdbid']})")
        ...
        for torrent in torrents:
            ...
            if current_state == self.db.states.SEEDING:
                ...
                if time_since_added_sec > self.retention_preiod_sec:
                    ...
                    new_state=self.db.states.COMPLETED
            # Change the torrent state if it finished the download and it is now uploading
            elif current_state == self.db.states.DOWNLOADING and torrent["state"] == "uploading":
                new_state=self.db.states.SEEDING
            # Stop download for torrents stopped
            elif current_state == self.db.states.PAUSED and not 'paused' in torrent["state"].lower():
                self.qbit.stop(hash)
            # Remove torrent marked for removing
            elif current_state == self.db.states.DELETING:
                self.qbit.delete(hash)
                self.db.delete_movie(id)

            if new_state != current_state:
                self._update_db_entry_state(id, type, new_state)
